...
const Select = React.forwardRef(({
    ...
    const getSelectedDisplay = () => {
        if (value === null || value === undefined || (multiple && value.length === 0)) {
             return placeholder;
        }
    ...
    const handleOptionSelect = (option) => {
        if (multiple) {
            ...
            onChange?.(updatedValue); // Devuelve el array de valores
        } else {
            onChange?.(option?.value); // Devuelve solo el valor
            setIsOpen(false);
            onOpenChange?.(false);
    ...
    const handleClear = (e) => {
        e?.stopPropagation();
        onChange?.(multiple ? [] : undefined); // Usar undefined o null para limpiar
    };
    ...
    const hasValue = multiple ? value?.length > 0 : value !== undefined && value !== null && value !== '';
            ...
            <div className="relative">
                <button
                    ...
                    className={cn(
                        ...
                        // --- ESTILO ACTUALIZADO ---
                        "bg-background text-foreground", // Coincide con Input.jsx
                ...
                <select
                    ...
                    onChange={() => { }} // Controlado por nuestra lógica
                    className="sr-only"
                    tabIndex={-1}
                    multiple={multiple}
                    required={required}
                ...
                {isOpen && (
                    <div className={cn(
                        "absolute z-50 w-full mt-1 border border-border rounded-md shadow-md",
                         // --- ESTILO ACTUALIZADO ---
                        "bg-card text-card-foreground" // Coincide con el tema
                    )}>
                        {searchable && (
                                ...
                                filteredOptions?.map((option) => (
                                    <div
                                        ...
                                        className={cn(
                                            ...
                                            // Ajuste para mejor contraste en selección
